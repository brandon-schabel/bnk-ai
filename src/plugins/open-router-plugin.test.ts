...
import { OpenRouterPlugin } from "./open-router-plugin";
...
describe("OpenRouterPlugin", () => {
    it("should parse SSE lines correctly and return content", async () => {
        ...
        try {
            const plugin = new OpenRouterPlugin("fakeKey");
    ...
    it("should throw if the fetch fails", async () => {
        ...
        try {
            ...
            await expect(plugin.prepareRequest(params)).rejects.toThrow(/OpenRouter API error/);
        } finally {
            globalThis.fetch = originalFetch;
        }
    });
